...
class Usuario{ //el nombre de la clase comienza con mayúscula y en singular.
  ...
  */
  class Libro {
    ...
    mostrarLibro(){
      console.log(`El libro ${this.titulo} con ISBN ${this.isbn} creado por el autir ${this.autor} tiene ${this.numpag} páginas`);
    }
    compararPaginas(Libro){
      if(this.numpag>Libro.numpag){
        let cantidadPaginasIf = this.numpag - Libro.numpag;
        return `El libro ${this.titulo} tiene ${cantidadPaginasIf} que el libro ${Libro.titulo}`
      }
      if(this.numpag<Libro.numpag){
        let cantidadPaginasIf = Libro.numpag -this.numpag
        return `El libro ${Libro.titulo} tiene ${cantidadPaginasIf} que el libro ${this.titulo}`
      }
      return 'Ambos libros tienen la misma cantidad de paginas'
    }
    //Libro.numPag, es el objeto que le vamos a mandar para comparar
  }
   let libro1 = new Libro('99-566-131-7','Cómo hacer amigos','Dale carnegie',303);
